//Egy tûzjelzõ beállításának formja
Unit TuzjelzoBeallUnit;

Interface

Uses Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
     StdCtrls, Buttons, ExtCtrls, FormFestoUnit, HozzaadUnit,
     BerendezesekLancUnit, TuzjelzoLancUnit, KapcsBerendLancUnit;

Type
  TTuzjelzoBeallDlg = Class(TForm)
    Bevel1      : TBevel;
    Label1      : TLabel;
    Label2      : TLabel;
    ComboBox1   : TComboBox;
  ...
  Public
    Property Tuzjelzo : TTuzjelzoLista Write TuzjelzoList;
...
Var TuzjelzoBeallDlg : TTuzjelzoBeallDlg;

Implementation

{$R *.DFM}

//egy tûzjelzõhöz kapcsolódó berendezések kiírása
Procedure TTuzjelzoBeallDlg.Beallit(Naploz: Boolean);
Begin
  ListBox1.Clear;
  NaplozBox.Checked:=Naploz;
  ListBox1.Items:=KapcsBerendList.KeresBerend(TuzjelzoList.Aktualis.TuzjelzoMutato);
End;

//kilépés
Procedure TTuzjelzoBeallDlg.CancelBtnClick(Sender: TObject);
Begin
  Close;
End;

//a form aktívvá válik
Procedure TTuzjelzoBeallDlg.FormActivate(Sender: TObject);
Begin
  RePaint;
...
//form átfestése
Procedure TTuzjelzoBeallDlg.FormPaint(Sender: TObject);
...
Procedure TTuzjelzoBeallDlg.Button1Click(Sender: TObject);
Begin
  Self.Visible:=False;
...
//kattintás a gombra
Procedure TTuzjelzoBeallDlg.OKBtnClick(Sender: TObject);
Var I      : Integer;
    Valtoz : Boolean;
Begin
  Valtoz:=False;
  //ha a lánc nem üres és van aktuális elem kiválasztva, ami módosítva lett
  If (TuzjelzoList <> Nil) And (ComboBox1.Text <> '') And Modositas Then
  Begin
    If (TuzjelzoList.Aktualis.Naplozas <> NaplozBox.Checked) Then
    Begin
      Valtoz:=True;
      TuzjelzoList.Aktualis.Naplozas:=NaplozBox.Checked;
    End;
    //kapcsolatok lánc frissítése
    KapcsBerendList.Kifuz(TuzjelzoList.Aktualis.TuzjelzoMutato);
    If (Self.MutTomb <> Nil) Then
    Begin
      Valtoz:=True;
      //berendezések felfûzése
      For I:=0 To High(Self.MutTomb) Do
        KapcsBerendList.Felfuz(MutTomb[I],TuzjelzoList.Aktualis.TuzjelzoMutato);
    End;
    If Valtoz Then TuzjelzoList.Valtozas:=True;
  End;
End;

//elem választása
Procedure TTuzjelzoBeallDlg.ComboBox1Change(Sender: TObject);
Var ID     : Integer;
    Mutato : PTuzjelzoElem;
Begin
  //azonosító keresése
  ID:=BerendList.KeresNev(ComboBox1.Items[ComboBox1.ItemIndex]);
  //azonosító alapján a mutató megkeresése
  If TuzjelzoList.KeresID(ID,Mutato) Then
  Begin
    TuzjelzoList.Aktualis:=Mutato;
...
Procedure TTuzjelzoBeallDlg.NaplozBoxClick(Sender: TObject);
Begin
  Self.Modositas:=True;
...
Procedure TTuzjelzoBeallDlg.Label2Click(Sender: TObject);
Begin
...
Procedure TTuzjelzoBeallDlg.AlkalmazBtnClick(Sender: TObject);
Begin
  OKBtnClick(Sender);
  AlkalmazBtn.Enabled:=False;
  Self.Modositas:=False;
