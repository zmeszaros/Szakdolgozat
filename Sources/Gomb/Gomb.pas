...
{ TKapcsolo }
  ...
  TKapcsolo = class(TCustomControl)
  private
    FActive: Boolean;
...
procedure Register;
begin
  RegisterComponents('Kapcsoló', [TKapcsolo]);
end;
...
constructor TKapcsolo.Create(AOwner: TComponent);
var
  I: Byte;
begin
...
procedure TKapcsolo.CreateParams(var Params: TCreateParams);
begin
...
procedure TKapcsolo.DefineProperties(Filer: TFiler);

{$IFDEF WIN32}
  function DoWrite: Boolean;
  begin
    if Assigned(Filer.Ancestor) then
      Result := FUserBitmaps <> TKapcsolo(Filer.Ancestor).FUserBitmaps
...
procedure TKapcsolo.GlyphChanged(Sender: TObject);
var
  I: Boolean;
begin
...
procedure TKapcsolo.SetSwitchGlyph(Index: Integer; Value: TBitmap);
begin
...
procedure TKapcsolo.CMFocusChanged(var Message: TCMFocusChanged);
var
  Active: Boolean;
begin
...
procedure TKapcsolo.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TKapcsolo.CMTextChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TKapcsolo.CMDialogChar(var Message: TCMDialogChar);
begin
...
procedure TKapcsolo.MouseDown(Button: TMouseButton;
...
procedure TKapcsolo.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TKapcsolo.DoOn;
begin
  if Assigned(FOnOn) then FOnOn(Self);
end;

procedure TKapcsolo.DoOff;
begin
  if Assigned(FOnOff) then FOnOff(Self);
end;

procedure TKapcsolo.ToggleSwitch;
begin
  StateOn := not StateOn;
end;

procedure TKapcsolo.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TKapcsolo.SetStateOn(Value: Boolean);
begin
...
procedure TKapcsolo.SetTextPosition(Value: TTextPos);
begin
...
procedure TKapcsolo.SetShowFocus(Value: Boolean);
begin
